...
const transport = new PlexHttpTransport({
  ...
  token: async () => token,
  clientIdentifier: 'vynode-laptop-season-collection-smoke',
  product: 'Vynode Laptop Season Smoke Test',
});
const identity = (await transport.query('/'))?.MediaContainer;
if (identity?.friendlyName !== 'Laptop')
  throw new Error('Season smoke test is restricted to Laptop.');
...
const payload = await transport.query(
  `/library/sections/${encodeURIComponent(libraryId)}/all?type=3&sort=titleSort&X-Plex-Container-Start=0&X-Plex-Container-Size=2`
);
const seasons = Array.isArray(payload?.MediaContainer?.Metadata)
  ? payload.MediaContainer.Metadata
  : [];
const memberKeys = seasons
  .map((item) => String(item?.ratingKey ?? ''))
  .filter(Boolean)
  .slice(0, 2);
if (memberKeys.length < 2)
  throw new Error('Laptop TV library requires at least two seasons.');
...
const title = `Vynode Season Integration Test ${Date.now()}`;
let ratingKey;
try {
  ratingKey = await client.create({ title, libraryId, mediaType: 'season' });
  const addition = await client.addMembers(ratingKey, memberKeys);
  if (addition.failures.length)
    throw new Error(`Failed to add ${addition.failures.length} season members.`);
  ...
  if (
    ...
    throw new Error('Temporary season collection verification did not match.');
