...
test('reports specific background collection failures for application logging',async()=>{const directory=await mkdtemp(join(tmpdir(),'vynode-job-log-'));try{const repository=new FileDurableJobRepository(join(directory,'queue.json'));const failures:string[]=[];const collections={async synchronize(){throw new Error('Plex returned HTTP 500.');}} as unknown as ProductionCollectionSurface;const runner=new ProductionCollectionJobRunner(repository,collections,()=>new Date(),(id,message)=>failures.push(`${id}: ${message}`));await assert.rejects(()=>runner.execute('broken'),/HTTP 500/);assert.equal(failures.length,3);assert.equal(failures[0],'broken: Plex returned HTTP 500.');}finally{await rm(directory,{recursive:true,force:true});}});
